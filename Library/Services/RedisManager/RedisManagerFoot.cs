...
namespace Services.RedisManage
{
    ...
    public class RedisManagerFoot : IRedisManagerFoot
    {
        private readonly RedisConnectionFoot _RedisCon;
        private readonly ConfigItems _config;
        public RedisManagerFoot(RedisConnectionFoot RedisCon, ConfigItems config)
        {
            _RedisCon = RedisCon;
            _config = config;
        }
        ...
        {
            IDatabase db;
            var s = ConfigItems.RedisServerFoot[serverr].Split('|');
            ConfigItems.Redd = s[0];
            db = RedisConnectionFoot.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
        ...
        public async Task<IList<T>> GetAllkeysData<T>(string pattern, int serverr = 1)
        {
            ...
            ConfigItems.Redd = s[0];
            db = RedisConnectionFoot.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
            var keys = await RedisManagerFoot.GetSetMembers(pattern, serverr);
        ...
        {
            var s = ConfigItems.RedisServerFoot[serverr].Split('|');
            return RedisConnectionFoot.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
        }
        #endregion
    }
}
