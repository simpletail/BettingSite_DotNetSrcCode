...
namespace Services.RedisManage
{
    ...
    public class RedisManagerTenni : IRedisManagerTenni
    {
        private readonly RedisConnectionTenni _RedisCon;
        private readonly ConfigItems _config;
        public RedisManagerTenni(RedisConnectionTenni RedisCon, ConfigItems config)
        {
            _RedisCon = RedisCon;
            _config = config;
        }
        ...
        {
            IDatabase db;
            var s = ConfigItems.RedisServerTenni[serverr].Split('|');
            ConfigItems.Redd = s[0];
            db = RedisConnectionTenni.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
        ...
        public async Task<IList<T>> GetAllkeysData<T>(string pattern, int serverr = 1)
        {
            ...
            ConfigItems.Redd = s[0];
            db = RedisConnectionTenni.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
            var keys = await RedisManagerTenni.GetSetMembers(pattern, serverr);
        ...
        {
            var s = ConfigItems.RedisServerTenni[serverr].Split('|');
            return RedisConnectionTenni.ConnectionLocal.GetDatabase(Convert.ToInt32(s[1]));
        }
        #endregion
    }
}
